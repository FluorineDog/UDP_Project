...
__global__ void kernel(int i, float *image_data, int *point_count,
	...
	// note: a pixel_index is treat as a pixel
...
cudaError_t precalcWithCuda(short *dev_data_samples_in_process, int ele_emit_id,
														...
														float *dev_filterdata, float *dev_imagedata,
														int *dev_pointcount, int parallel_emit_sum) {
	cudaError_t cudaStatus;

	//kernel 1,kernel2 decode
	//kernel3 filter
	cudaMemset(dev_filterdata, 0,
						 NSAMPLE * ELE_NO * sizeof(short) * parallel_emit_sum * 2);
	filter_func<<<4 * parallel_emit_sum, 512>>>(dev_filterdata,
	...
	calc_func<<<gridimage, 2 * M>>>(
			ele_emit_id, dev_imagedata, dev_pointcount, dev_filterdata,
...
int main(int argc, char const *argv[]) {
	...
	if (cudaMalloc((void **)(&dev_filterdata), length_of_data_in_process * 2) !=
			cudaSuccess)	// 转 float 乘以 2
	{
	...
	//for (i=1;i<=1;i++)
	{
		...
		cudaStatus = precalcWithCuda(
				...
				dev_filterdata, dev_imagedata, dev_pointcount, parallel_emit_sum);
		//}
		// over=time(NULL);
	...
	cudaStatus = cudaMemcpy(image_point_count, dev_sumpoint, N * N * sizeof(int),
													cudaMemcpyDeviceToHost);
	if (cudaStatus != cudaSuccess) {
